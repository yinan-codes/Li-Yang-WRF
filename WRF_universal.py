...
# event_type = 'weak'; # 事件类型，可选保存npy文件用
...
rlon1, rlat1 = threshold(rlon0, rlat0, wn_min, wn_max, conver, time_step, velo_threshold, 
                          rzwn=rzwn, rmwn=rmwn, wn_threshold=0.7, check_wn=True) # 是否进行经向波数制约
                         # check_wn=False)

#设置区域参数 格式为lon_min; lon_max; lat_min; lat_max 注意需要按照-360至720的经度来（三圈Mercator地球坐标）； ## cut = True表示截断；False保留完整轨迹 
...
lon, lat, WRF_u, WRF_v, ray_count, wave_propagation_time, wave_propagation_u, wave_propagation_v = WRF_calu(rlon_selected, rlat_selected, a, space, conver, time_step)
